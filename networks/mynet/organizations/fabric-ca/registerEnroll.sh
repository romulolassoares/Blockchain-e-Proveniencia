...
function createOrg3() {
  ...
  for i in {0..1}; do
    ...
    infoln "Registering peer${i}"
    set -x
    fabric-ca-client register --caname ca-org3 --id.name peer${i} --id.secret peer${i}pw --id.type peer --tls.certfiles ${PWD}/organizations/fabric-ca/org3/tls-cert.pem
    { set +x; } 2>/dev/null
    ...
    infoln "Generating the peer${i} msp"
    set -x
    fabric-ca-client enroll -u https://peer${i}:peer${i}pw@localhost:9054 --caname ca-org3 -M ${PWD}/organizations/peerOrganizations/org3.example.com/peers/peer${i}.org3.example.com/msp --csr.hosts peer${i}.org3.example.com --tls.certfiles ${PWD}/organizations/fabric-ca/org3/tls-cert.pem
    { set +x; } 2>/dev/null
    ...
    infoln "Generating the peer${i}-tls certificates"
    set -x
    fabric-ca-client enroll -u https://peer${i}:peer${i}pw@localhost:9054 --caname ca-org3 -M ${PWD}/organizations/peerOrganizations/org3.example.com/peers/peer${i}.org3.example.com/tls --enrollment.profile tls --csr.hosts peer${i}.org3.example.com --csr.hosts localhost --tls.certfiles ${PWD}/organizations/fabric-ca/org3/tls-cert.pem
    { set +x; } 2>/dev/null
